...
export default function RejectedIdeas() {
  ...
  useEffect(() => {
    const fetchIdeas = async () => {
      const q = query(collection(db, "rejectedIdeas"))
  ...
  const handleRevertIdea = async (ideaId: string, baInfo: { domainId: string; name: string }) => {
    ...
    try {
      ...
      await addDoc(collection(db, "pendingIdeas"), {
        ...idea,
        revertHistory: [...(idea.revertHistory || []), revertEntry],
      })

      await deleteDoc(doc(db, "rejectedIdeas", ideaId))
  ...
  const filteredIdeas = ideas
    .filter((idea) => {
      ...
      const matchesDateFilter = (() => {
        ...
        const ideaDate = new Date(idea.rejectedDate || idea.submissionDateTime)
  ...
  const handleExport = () => {
    try {
      ...
      link.setAttribute("download", `rejected_ideas_${new Date().toISOString().slice(0, 10)}.csv`)
  ...
  const stats = {
    total: ideas.length,
    thisMonth: ideas.filter((idea) => {
      const ideaDate = new Date(idea.rejectedDate || idea.submissionDateTime)
      const now = new Date()
      return ideaDate.getMonth() === now.getMonth() && ideaDate.getFullYear() === now.getFullYear()
    }).length,
    withRevertHistory: ideas.filter((idea) => idea.revertHistory && idea.revertHistory.length > 0).length,
              ...
              <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-gray-800">
                      {focusedIdea.rejectedDate ? new Date(focusedIdea.rejectedDate).toLocaleString() : "N/A"}
                    </p>
                  </div>
                  <div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-red-50 via-white to-orange-50">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-800 mb-2">Rejected Ideas</h1>
            <p className="text-gray-600">Review and manage rejected innovation ideas</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-gradient-to-r from-red-500 to-red-600 text-white p-6 rounded-2xl shadow-lg">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-red-100 text-sm">Total Rejected</p>
          ...
          <div className="bg-gradient-to-r from-yellow-500 to-yellow-600 text-white p-6 rounded-2xl shadow-lg">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-yellow-100 text-sm">With Revert History</p>
                <p className="text-3xl font-bold">{stats.withRevertHistory}</p>
              </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-2">
                ...
                <span className="bg-red-100 text-red-800 px-3 py-1 rounded-full font-bold">{ideas.length}</span>
              </div>
              <div className="flex items-center gap-2">
                  ...
                  <React.Fragment key={idea.id}>
                    <tr className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-4 text-sm text-gray-700">
                        {idea.rejectedDate ? new Date(idea.rejectedDate).toLocaleDateString() : "N/A"}
                      </td>
                      <td className="px-4 py-4">
                    ...
                    {expandedRows[idea.id] && (
                      <tr>
                        <td colSpan={8} className="px-0 py-0">
                          <div className="bg-gradient-to-r from-red-50 to-orange-50 p-6 border-t">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                              <div>
                              ...
                              <div>
                                <h4 className="font-semibold text-gray-700 mb-2">Rejected By</h4>
                                <p className="text-sm text-gray-600">
                                  {idea.approverName} ({idea.approverDomainId})
                                </p>
                              </div>
                              <div>
                                <h4 className="font-semibold text-gray-700 mb-2">Full Rejection Reason</h4>
                                <p className="text-sm text-gray-600">{idea.rejectionComment}</p>
                              </div>
                            </div>
