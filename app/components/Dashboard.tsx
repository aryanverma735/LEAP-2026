...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      const approvedQuery = query(collection(db, "approvedIdeas"))
      const rejectedQuery = query(collection(db, "rejectedIdeas"))
      const pendingQuery = query(collection(db, "pendingIdeas"))
      ...
      const approvedIdeas = approvedSnapshot.docs.map((doc) => doc.data())
      const rejectedIdeas = rejectedSnapshot.docs.map((doc) => doc.data())
      const pendingIdeas = pendingSnapshot.docs.map((doc) => doc.data())
