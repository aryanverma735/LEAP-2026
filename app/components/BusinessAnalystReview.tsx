...
import { collection, query, getDocs, doc, deleteDoc, addDoc, getDoc } from "firebase/firestore"
...
export default function BusinessAnalystReview() {
  ...
  const fetchIdeas = async () => {
    try {
      setLoading(true)
      setError(null)
      console.log("Fetching ideas from pendingIdeas collection...")

      const q = query(collection(db, "pendingIdeas"))
      const querySnapshot = await getDocs(q)
      const fetchedIdeas: Idea[] = []

      querySnapshot.forEach((doc) => {
        const data = doc.data()
        console.log("Found idea:", data.ideaId || doc.id)
        fetchedIdeas.push({ id: doc.id, ...data } as Idea)
      })

      console.log(`Total ideas fetched: ${fetchedIdeas.length}`)
  ...
  const handleApprove = async (id: string) => {
    ...
    try {
      const ideaRef = doc(db, "pendingIdeas", id)
      const ideaSnapshot = await getDoc(ideaRef)
      const ideaData = ideaSnapshot.data() as Idea
      await addDoc(collection(db, "approvedIdeas"), {
        ...ideaData,
  ...
  const handleReject = async (id: string) => {
    ...
    try {
      ...
      const ideaSnapshot = await getDoc(ideaRef)
      const ideaData = ideaSnapshot.data() as Idea
      await addDoc(collection(db, "rejectedIdeas"), {
        ...ideaData,
        approverDomainId: approverData.domainId,
        approverName: approverData.name,
        rejectionComment,
