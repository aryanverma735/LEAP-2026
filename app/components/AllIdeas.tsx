...
export default function AllIdeas() {
  ...
  useEffect(() => {
    const fetchIdeas = async () => {
      const approvedIdeasQuery = query(collection(db, "approvedIdeas"))
      const rejectedIdeasQuery = query(collection(db, "rejectedIdeas"))
      const pendingIdeasQuery = query(collection(db, "pendingIdeas"))

      const [approvedSnapshot, rejectedSnapshot, pendingSnapshot] = await Promise.all([
        getDocs(approvedIdeasQuery),
        getDocs(rejectedIdeasQuery),
        getDocs(pendingIdeasQuery),
      ])

      const approvedIdeas: Idea[] = []
      const rejectedIdeas: Idea[] = []
      const pendingIdeas: Idea[] = []

      approvedSnapshot.forEach((doc) => {
        approvedIdeas.push({ id: doc.id, ...doc.data() } as Idea)
      })

      rejectedSnapshot.forEach((doc) => {
        rejectedIdeas.push({ id: doc.id, ...doc.data() } as Idea)
      })
