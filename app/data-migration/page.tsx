...
export default function DataMigrationPage() {
  ...
  const runMigration = async () => {
    if (!confirm("Are you sure you want to run the data migration? This will update existing records in Firebase.")) {
      return
    }
      ...
      <div className="bg-white rounded-b-2xl shadow-xl p-8 space-y-8">
        ...
        {previewResult && (
          <div className="card bg-blue-50">
            ...
            {Object.entries(previewResult.collections).map(([collectionName, data]: [string, any]) => (
              <div key={collectionName} className="mb-6">
                <h4 className="font-medium text-gray-700 mb-2">
                  {collectionName} ({data.totalDocs} total documents)
                </h4>

                {data.changesPreview.length > 0 ? (
                  <div className="space-y-2">
                    <p className="text-sm text-blue-600 font-medium">
                      {data.changesPreview.length} documents will be updated:
                    </p>
                    <div className="max-h-40 overflow-y-auto space-y-1">
                      {data.changesPreview.map((change: any, index: number) => (
                        <div key={index} className="text-xs bg-white p-2 rounded border">
                          <div className="font-mono text-gray-600">Doc ID: {change.docId}</div>
                  ...
                  <p className="text-sm text-gray-600">No changes needed for this collection.</p>
                )}
              </div>
            ))}
          </div>
        ...
        {migrationResult && (
          <div className="card">
            ...
            {Object.entries(migrationResult.results).map(([collectionName, result]: [string, any]) => (
              <div key={collectionName} className="mb-4 p-4 border rounded-lg">
                <h4 className="font-medium text-gray-700 mb-2">{collectionName}</h4>

                <div className="flex items-center gap-2 mb-2">
                  <CheckCircle className="h-4 w-4 text-green-600" />
                  <span className="text-sm">Updated {result.updatedCount} documents</span>
                </div>
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          ...
          <ul className="text-sm text-yellow-700 space-y-1 ml-6">
            <li>• This migration will update existing records in Firebase</li>
            ...
            <li>• All three collections (pendingIdeas, approvedIdeas, rejectedIdeas) will be updated</li>
          </ul>
        </div>
      </div>
    </div>
